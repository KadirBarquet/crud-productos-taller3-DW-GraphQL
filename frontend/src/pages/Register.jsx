...
import { Mail, Lock, User, UserPlus, Database, Zap } from 'lucide-react';
...
const Register = () => {
    ...
    // GraphQL Registro
    const [registroGraphQL, { loading: loadingGraphQL }] = useMutation(REGISTRO, {
        onCompleted: (data) => {
            if (data.registro.success) {
                setTempUserData({
                    usuario: data.registro.usuario,
                    token: data.registro.token
                });
    ...
    const handleSubmit = async (e) => {
        ...
        // Intentar registro con GraphQL primero
        try {
            await registroGraphQL({
                variables: {
            ...
            try {
                ...
                toast.error(restError.response?.data?.message || 'Error al registrarse');
            } finally {
    ...
    const handleModeSelection = (selectedMode) => {
        ...
        toast.success(`¡Cuenta creada! Usando ${selectedMode === 'rest' ? 'REST API' : 'GraphQL'}`);
    ...
    // Modal de selección de modo después del registro
    if (showModeSelector) {
        return (
                ...
                <div className="max-w-4xl w-full">
                    <div className="text-center mb-8">
                        ...
                        <p className="text-lg text-gray-600">
                            Tu cuenta ha sido creada. ¿Qué modo de API prefieres usar?
                        </p>
                    </div>
